...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($sbalance < $_POST['amount']) {
        $_SESSION['error'] = "Недостатньо коштів на вашему рахунку!";
        echo "<script>history.back();</script>";
        exit();
    }
        ...
        if ($stmt->num_rows > 0) {
            ...
            if($balance == null){
                ...
                if ($stmt->affected_rows) {
                    $_SESSION["error"] = "Рахунок був додано " . $_POST['r-name'] . " was added!";
                }
            }
            ...
            if ($conn->affected_rows == 1){
                $_SESSION["success"] = "Транкзація пройшла успішно!";
            }else{

                $_SESSION["error"] = "Не вийшло провести транкзакцію!";
            ...
            if ($history->affected_rows) {
                ...
                $_SESSION["error"] = "Не вдалось оновити в історії!";
                header("location: ../pages/project/operations.php");
                exit();
            }
            ...
            // Incorrect username
            $_SESSION['error'] = "Невірний ІD акаунта!";
            echo "<script>history.back();</script>";
            exit();
    ...
    $_SESSION["error"] = "Заповіть всі поля!";
    header("location: ../pages/project/operations.php");
    exit();
}
?>